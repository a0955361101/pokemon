...
<template>
    ...
    <h1 class="title">{{titleName}}</h1>
        <div class="attributes-wrap">
            <div v-for="item in attributes" class="attributes-item" :class="item.className" :key="item.id" @click="changeAttributes(item.id)">
    ...
    <div v-for="item in showAttackEffectArr" :key="item" class="Xiangke-wrap">
            <div class="Xiangke-item-wrap excellent-effect">效果絕佳 
                <div v-for="doubleShow in item.attributeRestraint.double" class="attributes-item Xiangke-item" :class="handleClassNameShow(doubleShow)">
                    <div class="attributes-attribute"></div>
                    <p>{{(handleAttributesShow(doubleShow))}}</p>
                </div>
            </div>
            <div class="Xiangke-item-wrap no-effect">沒有效果 
                <div v-for="zeroShow in item.attributeRestraint.zero" class="attributes-item Xiangke-item" :class="handleClassNameShow(zeroShow)">
                    <div class="attributes-attribute"></div>
                    <p>{{(handleAttributesShow(zeroShow))}}</p>
                </div>
            </div>
            <div class="Xiangke-item-wrap ineffective">效果不好 
                <div v-for="halfShow in item.attributeRestraint.half" class="attributes-item Xiangke-item" :class="handleClassNameShow(halfShow)">
                    <div class="attributes-attribute"></div>
